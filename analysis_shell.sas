...
/*  **************************************************************************************************************  */
    ...
    %DO n_var=1 %TO %words(&varlist);
        ...
        %IF %upcase(&curr_vartype) = CATEGORICAL %THEN %DO;
            ...
            %categoricaltest_chisq(dataset=&dataset, groupvar=&groupvar, var=&curr_var, out=&curr_var.test)
        ...
        %IF %upcase(&curr_vartype) = LOGRANK %THEN %DO;
            ...
            %logranktest(dataset=&dataset, groupvar=&groupvar, var=&curr_var, timevar=&curr_var.days, groupvar_refindex=&groupvar_refindex, out=&curr_var.test)
